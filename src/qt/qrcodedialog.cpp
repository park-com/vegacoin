    ...
    address(addr)
{
    ...
    if (label.trimmed().isEmpty()) {
        setWindowTitle(QString("Vegacoin - Generate QR Code"));
    } else {
        setWindowTitle(QString("Vegacoin - Generate QR Code for %1").arg(label));
    }
...
{
    QString ret = QString("vegacoin:%1").arg(address);
    int paramCount = 0;
