...
const string strMessageMagic = "Vegacoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if (pindexBest->nHeight == 1)
		nSubsidy = 60000000 * COIN;  // 10% Premine	
	else if (pindexBest->nHeight <= FAIR_LAUNCH_BLOCK) // Block 500 Instamine prevention
        nSubsidy = 1 * COIN;	 // 50,000 VEGA for instamine prevention
	else if (pindexBest->nHeight <= 1000000) // 298,500,000 VEGA
		nSubsidy = 300 * COIN;	
	else if (pindexBest->nHeight <= 2000000) // 200,000,000 VEGA
		nSubsidy = 200 * COIN;
	else if (pindexBest->nHeight <= 3000000) // 100,000,000 VEGA 
		nSubsidy = 100 * COIN;		
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Vegacoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "12/SEP/2017 VEGACOIN LAUNCH SUPPORTED BY THE VEGA GROUP ";
        CTransaction txNew;
        txNew.nTime = 1505168939;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
